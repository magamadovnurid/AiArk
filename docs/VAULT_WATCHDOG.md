...
If that permission prompt is deferred, the same installed helper can run in a detached user session as a temporary fallback: `screen -dmS aiark-vault-watchdog node "$HOME/Library/Application Support/AiArk/VaultWatchdog/vault-watchdog.mjs" watch`. Only start it after checking `screen -ls` for an existing watchdog. It checks every two minutes; the helper uses an interprocess lock so the fallback and LaunchAgent cannot restart the same queue together. The fallback survives terminal closure but not a Mac restart, unlike the LaunchAgent.
